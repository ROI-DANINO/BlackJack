...
| docs/superpowers/specs/2026-08-19-learning-design-blueprint.md | **Phase 4's gate artifact (`LDB-08`)** — the assembled learning design: what is taught (18 Skills / 3 Subjects), through which activities (11 types), measured by what evidence (8 of the last 10 table-closed), in what order (2 session types, 18 composition decisions), and why the learner returns (the chips economy). All 74 decisions from `LDB-01`–`LDB-06`, `LDB-09` and `LDB-10` appear with their evidence labels. **Authoritative for the phase-5 slice and the declared `P-1`/`P-3`/`P-5` subset it must instrument.** It assembles rather than re-decides: each source spec remains authoritative for its own rulings. |
