...
This file is fully regenerated by `web/qa/progress/run.ts` on every `qa:progress` run (a single
`writeFileSync`, no hand-maintained slot) — the durable T10-M2 delta/canonical-ratio observation
(this run's numbers, the prior run's comparison, and the non-zero pre-tier baseline) lives in
`journal/qa/ledger.md`'s "Learning-integrity contract detail" section precisely because a hand-edit
here would not survive the next re-run.
