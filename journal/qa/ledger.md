...
|------|---------------|----------|--------------------|---------|-------|
| Engine rules & math | `crates/` | 2026-07-11-get-to-know-blackjack | 44606e2 | PASS | `cargo test -p blackjack-core` passed; prefix-shoe/session tests cover composition-preserving arranged openings. Existing engine paths smoke-tested. |
| UI/wire fidelity (display ↔ engine ↔ JSONL) | `web/src/`, `crates/` wire types | 2026-07-11-get-to-know-blackjack | 44606e2 | PASS | `qa:rules` passed — 90 rounds re-derived, 12/12 situations ≥2×, 0 violations; `npm test` 144/144 passed. |
| Betting & payout math (bankroll, 3:2, push, split/double stakes) | `web/src/`, `crates/` | 2026-07-11-get-to-know-blackjack | 44606e2 | PASS | `qa:rules` passed — H17, payout multipliers, split/double scaling, bankroll chain re-derived per round, 0 discrepancies. |
| Round & state flow (turns, buttons, reset, reshuffle) | `web/src/` | 2026-07-11-get-to-know-blackjack | 44606e2 | PASS | `qa:flow` passed — 273 snapshots, 29 split-turn, 1 reshuffle, 0 violations; drill resolved split marker fixed as QA-015. |
| Guided drill learning loop | `web/src/drill/`, `web/src/app/Drill.tsx`, `web/src/breakit-hook.ts`, prefix-session engine path | 2026-07-11-get-to-know-blackjack | 44606e2 | PASS | `qa:drill` passed — 3/3 units, 5 arranged situations, 9 live hands, 0 violations; PX pass says short/playable with no strategy grading. |
| Player experience & clarity | `web/src/` | 2026-07-11-get-to-know-blackjack | 44606e2 | PASS-with-reservations | PX pass completed all three units; QA-015 fixed+verified; Hit-on-16 accepted as bust-risk orientation, not strategy advice. QA-014 V3 polish reservation remains. |
| Robustness (rapid input, illegal actions, refresh, long grind) | `web/src/` | 2026-07-11-get-to-know-blackjack | 44606e2 | PASS | `qa:breakit` passed — 12/12 attacks, 0 breaks. QA-008 (refresh wipe)/QA-012 open backlog remain non-blocking. |
...
| QA script suite (`web/qa/`: rules+flow+breakit+drill) | `web/qa/`, `web/src/` | 2026-07-11-get-to-know-blackjack | 44606e2 | PASS | `cd web && npm run qa` now runs rules → flow → breakit → drill; all four passed at 44606e2. Reports in `runs/2026-07-11-{rules,flow,breakit,drill}/`. |
