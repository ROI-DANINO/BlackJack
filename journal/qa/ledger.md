...
| QA-018 | 2026-07-15-agent-kanban | agent workflow & handoff | major | Local movement policy and mutating skills did not require demonstrable `Done when` satisfaction, and QA omitted Ready entry plus gated Done success/refusal. | **verified** (2026-07-15) — policy/skills fixed in `1f221be`; portable evidence in `6a4c05f`; focused Ready/Done/refusal report `25845d5f...` passed; board closed at `f12d580` |
| QA-019 | 2026-08-02-pr11-salvage (not a QA run — surfaced while auditing PR #11) | learning integrity | major | **Recap copy asserts learner capability that nothing conditions on evidence.** `web/src/learn/content/blackjack-basics.ts:290` reads `text: 'You can explain and use Hit.'`, and the pattern runs to **16 assertions across all 9 recap steps** (`:81-82`, `:159-161`, `:216-217`, `:290-291`, `:356-357`, `:422`, `:485`, `:548-549`, `:603`). Mechanism verified first-hand 2026-08-02: `web/src/learn/controller.ts` sets `awaitingContinue` for `explain` and `recap` steps, and `continue()` advances on plain acknowledgement — **nothing conditions reaching recap on correctness**, so a learner who answered every question wrong still lands on "You can explain and use Hit." This is Phase 2 verdict `K-U6-009` (Relabel → Product judgement), one of the three audit corrections whose locus is code and which the 2026-07-26 document sweep never reached. **The verdict names only `:290`; the other 15 instances are named nowhere.** | open — deferred to phase 5 **with a reason, not for convenience**: the honest fix conditions the copy on the mastery computation (claim a capability only where evidence supports it), which needs `LDB-04`'s reducer to exist. Phase 4 designs and builds nothing, and `AGENTS.md` scopes the board to the active milestone, so a phase-5 build task has no legal card. Logged here because it otherwise has no owner at all — `LDB-04` owns the principle, this owns the defect |
...
| 2026-08-17 | phase-4-blueprint | milestone QA — **scoped to nothing, recorded not run** | 6686e50 (unchanged since) | — | **No-op by ledger scoping, not by exemption.** Phase 4 produced no product code: 119 commits since `6686e50` (2026-07-19), of which **0** touch `crates/` or `web/` (`git rev-list --count 6686e50..HEAD -- crates/ web/`). Every coverage area's last-passed commit is at or before that point, so no area has a changed watched file and there is nothing to deep-test; a smoke run would re-stamp identical bytes. **No baseline advanced — every Last-passed commit above stands unchanged.** Phase 5's first slice therefore deep-tests against the phase-3 baseline, not against a phase-4 pass. Ruling: `journal/decisions.md` 2026-08-17. |
