...
| QA-019 | 2026-07-30-ldb-04-design (not a QA run — surfaced while drafting the mastery model) | learning integrity | major | **Recap copy asserts learner capability that nothing conditions on evidence.** `blackjack-basics.ts:290` reads `text: 'You can explain and use Hit.'`; the pattern runs to **16 assertions across all 9 recap steps** (`:81-82`, `:159-161`, `:216-217`, `:290-291`, …). Verified mechanism: recap is the terminal step reached by plain acknowledgement — `controller.ts:83` handles "a step that only needs acknowledgement (explain/recap)", `:139` marks it `awaitingContinue`, and **nothing conditions reaching it on correctness**, so a learner who answered every question wrong still lands on "You can explain and use Hit." This is Phase 2 verdict `K-U6-009` (Relabel → Product judgement, "asserts learner capability and is never conditioned on evidence"), one of three audit corrections whose locus is code and which the 2026-07-26 sweep never reached. **The verdict names only `:290`; the other 15 instances are unnamed anywhere.** | open — deferred to phase 5 **with a reason, not for convenience**: the honest fix conditions recap copy on the mastery computation (show the capability claim only when evidence supports it, otherwise "covered: Hit"), and that needs the `LDB-04` reducer to exist. Phase 4 designs and builds nothing, and `AGENTS.md` bars a phase-5 build task from the LDB board. Logged here because it had **no owner card at all** — `LDB-04` §6.3 states it is out of that card's scope. |
