...
|------|---------------|----------|--------------------|---------|-------|
| Engine rules & math | `crates/` | 2026-07-15-strategy-profile-foundation | 84953ed | PASS | **Deep-tested.** `cargo fmt --all -- --check`, `cargo clippy -p blackjack-core -- -D warnings`, and `cargo test -p blackjack-core` all passed (92 tests). Exact canonical H17/S17 resolution, altered-ruleset rejection, both source charts, and JSON compatibility verdicts are covered. |
| UI/wire fidelity (display ↔ engine ↔ JSONL) | `web/src/`, `crates/` wire types | 2026-07-15-strategy-profile-foundation | 84953ed | PASS | **Deep-tested.** `npm test` passed 215 tests. Real-WASM lifecycle coverage in `web/src/learn/controller.test.ts` verifies compatible → renderable, profile mismatch/unsupported → pre-render fatal, and the profile-less path. `qa:rules` passed 90 re-derived rounds / 514 assertions / 0 violations. |
| Betting & payout math (bankroll, 3:2, push, split/double stakes) | `web/src/`, `crates/` | 2026-07-15-strategy-profile-foundation | 84953ed | PASS | **Smoke-tested; payout behavior unchanged.** `qa:rules` again re-derived H17, payout multipliers, split/double scaling, and bankroll chains for 90 rounds with 0 discrepancies. |
| Round & state flow (turns, buttons, reset, reshuffle) | `web/src/` | 2026-07-15-strategy-profile-foundation | 84953ed | PASS | **Smoke-tested; round-flow behavior unchanged.** `qa:flow` passed: 273 snapshots, 1166 assertions, 29 split-turn snapshots, 1 reshuffle, 0 violations. |
| Blackjack Basics learning system | `web/src/learn/`, `web/src/app/{Learn,Lesson,useLesson}.tsx`, `web/src/breakit-hook.ts`, `web/qa/learn/` | 2026-07-15-strategy-profile-foundation | 84953ed | PASS | **Deep-tested.** Profile-less curriculum remains playable: `qa:learn` passed 9/9 units, six arranged + one live checkpoint hands, 28 assertions, 0 violations. Declared-profile gate lifecycle was re-run through the real WASM transport in `web/src/learn/controller.test.ts` (27/27 focused tests). Report `runs/2026-07-15-learn/`. |
...
| Robustness (rapid input, illegal actions, refresh, long grind) | `web/src/` | 2026-07-15-strategy-profile-foundation | 84953ed | PASS | **Smoke-tested; no robustness behavior changed.** `qa:breakit` passed all 12 attacks, 0 failed invariants, with the dev hook absent from the production build. QA-008/QA-012 remain non-blocking backlog. |
...
| QA script suite (`web/qa/`: rules+flow+breakit+learn) | `web/qa/`, `web/src/` | 2026-07-15-strategy-profile-foundation | 84953ed | PASS | `npm --prefix web run qa` passed rules → flow → breakit → learn at 84953ed; each role checked WASM freshness before its production preview. Reports in `runs/2026-07-15-{rules,flow,breakit,learn}/`. |
...
| 2026-07-15 | strategy-profile-foundation | scoped feature QA | 84953ed | `runs/2026-07-15-strategy-profile-foundation/` | PASS. Deep: engine, UI/wire, Blackjack Basics. Smoke: payout, flow, robustness; PX skipped because `web/src/app/` did not change. fmt, clippy, core 92 tests, web 215 tests, and full `npm run qa` all passed. Every QA role's WASM freshness guard passed; no rebuild was needed. Real-WASM controller lifecycle confirms compatible render, mismatch/unsupported pre-render fatal, and playable profile-less Basics. No new findings. |
