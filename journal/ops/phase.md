...
step: LDB-06 drafted 2026-07-30 and awaiting approval in Verification — session composition. The bridge contradiction the card required is RESOLVED IN WRITING with both loci verified: product-design-inputs.md:230 states rare-event exposure as requirement §4.6 while :294 lists it as open and cites :230, so neither section overlooked the other. Ruling: §4.6 stands, exposure is engineered, §7's entry withdrawn as mis-scoped, on three grounds — §4.6 rests on §1.2 [VERIFIED] across >70,000 choices while §7 offers no counter-evidence and an unwarranted 'open' does not reopen a warranted requirement; 'organic' is not the weaker option but the exact condition §1.2 measured and found wanting, since waiting for the shoe is sampling at the shoe's own frequencies; and approved outcome P4 needs an engineered losing run to be measurable at all, so organic would leave A-26 unfalsifiable. What was genuinely open was the dosage, now A-07c rather than a binary in a decision list. Session shape written against the existing schema (closeReason's four values, budget's presetId/targetDurationMs/maxActivities), from which two rules fall out rather than being imposed: time- and activity-bound sessions close at END OF ACTIVITY so a truncation is not recorded as an abandoned attempt, and learner-stopped forfeits nothing, which BINDS LDB-05 rather than advising it. Mix is the one place learner choice is refused, scoped against the navigation case: 78 percent performed better spaced while 78 percent rated massing as good or better, so preference is anti-correlated with benefit, and A-17 says confidence will not flag it. Found while writing: SessionRecord carries no pool-composition field, so P-3's arms are indistinguishable after the fact — additive, handed forward. Two register sub-rows A-07b, A-07c.
...
next: APPROVE OR REVISE LDB-06 — it is drafted and in Verification, not waiting to be started. docs/superpowers/specs/2026-07-30-session-composition.md. Approving unblocks LDB-07 and leaves only LDB-05 and the LDB-08 blueprint assembly before the phase-4 exit gate. THE WEAKEST PARTS, NAMED SO A REVIEWER STARTS THERE: the three session presets are invented and defended only by A-07b, and they deliberately SPAN the A-08 Duolingo-vs-Brilliant conflict rather than resolving it; the engineered-tail dosage and the P4 losing-run floor in A-07c are likewise invented. AFTER LDB-06, two cards remain and they suit different sessions. LDB-07 (interaction UX) continues naturally from LDB-03 and LDB-06 — it needs the seven families and the session shape in working memory, and it still owes the target WCAG conformance level plus the ALR-040 split marking the AAA element voluntary, which K-U4-016 landed as levels-per-criterion but not as a target. LDB-05 (economy) is the most independent card left: its input is 2026-07-26-chips-xp-and-progression-economy.md's E-1..E-7, unread this session, and it is now CONSTRAINED by LDB-06 §4 — no penalty for stopping, no loss framing, no voided evidence. LDB-08 is best done fresh: it must read all seven approved documents and check them against each other, which a clean read serves better than a long one.
