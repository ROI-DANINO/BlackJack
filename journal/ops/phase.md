...
sub_phase: strategy-table-fundamentals-design-approved
plan: null
spec: docs/superpowers/specs/2026-07-12-strategy-profile-foundation-design.md
step: "Strategy Profile Foundation (first Strategy Table Fundamentals slice) design APPROVED + written. Next: writing-plans. Blackjack Basics remains complete/merged (964efa1); its design/plan at docs/superpowers/{specs,plans}/2026-07-11-blackjack-basics-learning-foundation*."
...
detour: []
blocking: null
next: "writing-plans for the Strategy Profile Foundation slice. Then tasks.md gets task detail. Open call to Roi: whether the plan opens by dispatching the rules-researcher to verify the full 6D S17 DAS no-surrender table (a hard dependency) before the rest of the build. Revisit the Hit-on-16 orientation moment when the first actual strategy lesson is designed (next slice, per the design's Next Design Boundary)."
