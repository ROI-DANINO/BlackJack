...
next: Approve or revise LDB-03, the activity taxonomy and capability map (docs/superpowers/specs/2026-07-30-activity-taxonomy-and-capability-map.md). It is in Verification: drafted, adversarially examined, and all 11 non-Preserve findings landed with each verified present in the target file by positive enumeration (22/22). Approving unblocks LDB-04, LDB-06, LDB-07 and LDB-08 — everything left in the milestone. TWO THINGS TO WEIGH BEFORE APPROVING. First, the citation half was never verified by a second hand: the audit-verifier run was dispatched to a write path outside its enforced scope, died at that write, and its 12 figure/quote checks and count re-verification are lost. Those are mechanical greps against the LDB-02 archive and can be re-run cheaply; the examiner did independently check §5's disposition arithmetic and found it sound. Second, the examination changed the design and not merely the prose — F2 is now supporting rather than primary on A4 (A4 is behavioural, F2 measures declarative sequence knowledge), condition 1 is a supply test rather than a cardinality test and now carries a Product judgement label it previously lacked entirely, and §4.2 was decided by silence and is now contract C-F with the pool policy handed to LDB-06 by name. Also still open and owned elsewhere: K-U6-003/005/009 are unlanded in web/src, and the owner's decision to fix K-U6-005 by renaming the assistance level rather than annotating it is recorded on the LDB-04 card.
