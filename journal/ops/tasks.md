...
- Gate: user-approval
- Evidence: APPROVED 2026-07-30 — docs/superpowers/specs/2026-07-30-evidence-and-mastery-rules.md. Closes ROADMAP phase-4 deliverable 5. Mastery = accumulated evidence weight >= 3.0, with at least one unassisted item as a STRUCTURAL gate (no register row — there is no constant to calibrate), not decayed past a 14-day horizon. Weights by assistance x tableVisibility: 1.0 unassisted/hidden, 0.5 unassisted/open, 0.35 retry, 0.15 instruction, 0 incorrect/ungraded/abandoned — so a played hand at hidden visibility is the HIGHEST-weighted evidence in the model, which is what discharges the bridge's disqualifying constraint. Diagnostic-mode attempts stored but never folded, honouring the schema's own comment. KEY FINDING: a reducer, not a migration — progress/types.ts already carries evidence.skillId, assistance, tableVisibility, occurredAt ('Recency/spacing input'), attemptOrdinal, presentationId and AttemptKind including 'classification'; controller.ts:360-362 keys on stepId and discards outcomeId, already a validated FK into Subject.skills. Per-family evidence ceilings set: F1 mastery evidence for C1 only, F2 supporting-only on A4 (never sole evidence, since A4 is behavioural and F2 measures declarative sequence knowledge), F3/F4/F6/F7 full, F5 scored as calibration over a series rather than per-attempt. Gating: RECOMMEND-DO-NOT-LOCK, on the internal ground that hard gating would make A-01 load-bearing at runtime. K-U6-005 fixed by RENAMING to 'retry-2' in both Assistance unions plus a second unnamed instance at controller.ts:218, safe because verified nothing has ever been persisted; K-U6-003 retired rather than corrected; K-U6-009 out of scope and now logged as QA-019 with 16 instances found where the verdict names one. Register: two sub-rows only (A-06a horizon, A-07a weights+threshold+F5 bar pooled as one calibration target), per the owner's instruction that overlapping assumptions share instruments, with the non-independence reason stated. Kulik 1990 not opened — five routes blocked at the proxy (CONNECT tunnel failed 403), an environment policy block recorded as a named reachable gap, so the literature is labelled leads-not-evidence.
- Updated: 2026-07-30T11:46:33.410Z
