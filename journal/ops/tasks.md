...
- Load: docs/superpowers/research/evidence-index/activity-and-storage-catalog.md, docs/superpowers/specs/2026-07-22-product-design-inputs.md
- Gate: user-approval
- Evidence: DRAFT AWAITING APPROVAL — docs/superpowers/specs/2026-07-30-activity-taxonomy-and-capability-map.md. Seven families (three inherited, Multiple Choice demoted per A-10/K-U4-003; four added), six contracts C-A..C-F, all 18 outcomes mapped, all 32 LDB-02 patterns dispositioned 17 adopted / 11 rejected outright / 4 conditional-or-deferred. TWO REVIEW PASSES, BOTH BANKED. (1) Independent adversarial examination: docs/superpowers/audits/2026-07-30-ldb-03-claim-examination.md — 10 claims, 13 units, 2 Preserve / 7 Revise / 3 Relabel / 1 Replace. All 11 non-Preserve landed, each verified present in the target file by positive enumeration (22/22). It changed the design, not just the prose: condition 1 went from a cardinality test to the classifier's actual effect-equivalence-via-supply criterion and gained a Product judgement label it previously lacked entirely; the two conditions were re-attributed as this document's own test rather than an excavation of the classifier's supply-based reasoning; U3-7's sourced form is now stated as satisfying condition 2 and indeterminate on condition 1; B1 and A3 rejections re-grounded; A3's 'content is the soft-17 condition' struck as false against LDB-01's 'forced'; F2 demoted to supporting on A4 because A4 is behavioural and F2 measures declarative sequence knowledge; C-A relabelled as operationalising A-24 with its pseudo-quoted prescription deleted; P3's in-play half assigned to F3; A-29 registered for C-B's decoration claim; §4.2 discharged by new contract C-F with the pool policy handed to LDB-06 by name and F4-F7's absent requirement sets handed to LDB-07. (2) Citation verification: docs/superpowers/audits/2026-07-30-ldb-03-citation-verification.md — 12 of 12 figure/quote/locus claims CONFIRMED, 0 refuted, counts verified by enumeration against classification.md's own 32-id set. Anti-vacuity guard run on the zero-EV-machinery claim (crates/ confirmed present with 24 .rs files, positive control basic_strategy returns 7). CAVEAT ON STANDING: pass (1) is independent; pass (2) is the author re-running a lost verifier run by hand and closes a mechanical gap only, not a second hand on judgement.
- Updated: 2026-07-30T10:19:55.921Z
