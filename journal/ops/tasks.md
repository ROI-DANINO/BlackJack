...
- Next: FROM LDB-02, this card's confidence-is-never-a-mastery-signal ruling now has independent evidence: Kornell and Bjork, 78 percent performed better spaced while 78 percent rated massed as good or better (docs/superpowers/research/activity-pattern-catalog/classification.md item 3). U2-9 is the catalogued anti-pattern proposing exactly that widget — cite both in the decision. Also relevant: U1-6's rating-dependent grading breaks cross-learner comparability, a trap for any mastery constant; and U1-4 models assistance as a recorded input rather than a flag. OWNER DECISION 2026-07-30, carried here rather than actioned because the U6 corrections were scoped out of that turn: K-U6-005 is to be fixed by RENAMING the mislabelled assistance level, not merely annotating it. 'instruction' at web/src/learn/types.ts:40 records retry depth and no instruction is delivered; the rename must carry through controller.ts:123, the Assistance type, tests, and any persisted AttemptRecord — note journal/decisions.md:34 treats that schema as additive-only, so check the migration path before renaming a persisted value. K-U6-003 (validate.ts:65-66) and K-U6-009 (blackjack-basics.ts:290) are also still unlanded; neither has another owner. LDB-03 contract C-D forbids adding any second instance of the same mislabel.
...
- Gate: user-approval
- Evidence: DRAFT AWAITING APPROVAL — docs/superpowers/specs/2026-07-30-evidence-and-mastery-rules.md. Mastery = accumulated evidence weight >= 3.0, with at least one unassisted item as a STRUCTURAL gate (not a threshold, so no register row), not decayed past its horizon. Weight table by assistance x tableVisibility: 1.0 unassisted/hidden, 0.5 unassisted/open, 0.35 retry, 0.15 instruction, 0 for incorrect/ungraded/abandoned — which is how 'assisted counts but unassisted counts more' is expressed structurally rather than numerically. A played hand at hidden visibility is the HIGHEST-weighted evidence in the model, discharging the bridge's disqualifying constraint. KEY FINDING: this is a reducer, not a migration — progress/types.ts already carries evidence.skillId, assistance, tableVisibility, mode ('diagnostic is STORED but excluded from mastery — by the reducer, not the store'), occurredAt ('Recency/spacing input'), attemptOrdinal, presentationId and AttemptKind including 'classification'; the shipped controller.ts:360-362 keys on stepId and discards outcomeId, which is already a validated FK into Subject.skills. OWNER DECISIONS APPLIED: decay adopted with a 14-day horizon (A-06a) plus the owner's amendment that adjacent fields can test it — re-failure rate inside vs outside the horizon, computable from ProgressAttempt alone, no new instrument; assisted counts but weighted below unassisted; gating is RECOMMEND-DO-NOT-LOCK, on the internal ground that hard gating would make A-01 load-bearing at runtime when LDB-01 declined to assert that order even as graph edges; K-U6-005 fixed by RENAMING to 'retry-2' in both Assistance unions, safe because verified nothing has ever been persisted (only importer of progress/ outside its dir is a QA harness; zero appendAttempt calls in web/src), and a SECOND instance at controller.ts:218 is fixed though the audit verdict names only :123. K-U6-003 retired rather than corrected — validate.ts's question-step restriction is replaced by per-skill declared evidence. K-U6-009 left explicitly open, no owner. Register: TWO sub-rows only (A-06a, A-07a), pooled per the owner's instruction that overlapping assumptions share instruments, with the pooling reason stated — no weight is independently validatable against a threshold it helps define. Kulik 1990 NOT OPENED: five routes all failed at the proxy with CONNECT tunnel failed 403, an environment network-policy block rather than a paywall; recorded as a named reachable gap, so §4 rests on the internal A-01 argument and the literature is labelled leads-not-evidence.
- Updated: 2026-07-30T11:36:31.051Z
...
- Load: docs/superpowers/research/evidence-index/activity-and-storage-catalog.md, docs/superpowers/specs/2026-07-22-product-design-inputs.md
- Gate: user-approval
- Evidence: APPROVED 2026-07-30 — docs/superpowers/specs/2026-07-30-activity-taxonomy-and-capability-map.md. Closes ROADMAP phase-4 deliverables 3 and 4. Seven families as a CLOSED list (three inherited, Multiple Choice demoted per A-10/K-U4-003 so recognition is sole evidence for C1 only; four added because four LDB-01 outcomes had nothing that could measure them). Six cross-cutting contracts C-A..C-F. All 18 outcomes mapped; all 32 LDB-02 patterns dispositioned 17 adopted / 11 rejected outright / 4 conditional-or-deferred. Word-bank vs Parsons ruled once on two stated admission conditions: the option-set form must not pose the same question to the same effect (supply, not cardinality), and the pool must over-supply with distractors that must be rejected. TWO REVIEW PASSES, DIFFERENT STANDING. Independent adversarial examination (audits/2026-07-30-ldb-03-claim-examination.md): 10 claims, 13 units, 2 Preserve / 7 Revise / 3 Relabel / 1 Replace; all 11 non-Preserve landed and verified in-file 22/22. It changed the design, not the prose — condition 1 re-grounded from cardinality to supply and labelled Product judgement where it had carried no label at all; the conditions re-attributed to this document rather than to the classifier, whose recorded ground is supply not length; F2 demoted to supporting on A4 because A4 is behavioural; A3's soft-17 claim struck as false against LDB-01's 'forced'; C-A relabelled as operationalising A-24; §4.2 discharged by new contract C-F with the pool policy handed to LDB-06 by name. Author-run citation pass (audits/2026-07-30-ldb-03-citation-verification.md): 12 of 12 figures, quotes and loci CONFIRMED, counts enumerated against classification.md's own 32-id set, anti-vacuity guard on the zero-EV-machinery claim — closes a mechanical gap, not a second hand on judgement.
- Updated: 2026-07-30T10:24:09.057Z
