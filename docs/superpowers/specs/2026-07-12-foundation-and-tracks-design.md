...
> Note: this parity/freshness item is the one genuinely-earned *technical* entry in the table — small in size, real in evidence. Treat it as **scheduled hardening, not a standing seam**. It was scheduled for the S17 `CoreCommand` slice but did not land; it now rides the next boundary-touching grading slice.
...
**The real fork Roi must eventually call (see §11):** server **authority** (anti-cheat, authoritative money/leaderboards) vs server **persistence + sync** of a client-authoritative trainer. A stakes-free trainer may never need authority — which would make redaction and server-authority replay/security work permanently deferrable and collapse "cloud" to *auth-at-the-edge + the existing `LogSink` pattern*. Native↔built-WASM artifact parity remains current hardening either way.
...
4. **Carry the missed cheap pain-family hardening into the next boundary slice:** extend the freshness guard to `Cargo.lock` + `build-wasm.sh`, and add a native↔wasm32 parity run over the real artifact. It was scheduled with S17 but did not land.
