...
- **The interface already exists and is minimal:** one stateless `dispatch_json(&str)->String`, reached through the TS adapter layer — chiefly `engine.ts`, with `situations.ts` also parsing raw JSON. Neither re-derives domain truth; they carry engine facts across. Seven commands, three response variants, a `{status: ok|error}` envelope, two-tier errors (`CoreRuleError` recoverable vs `BridgeError` fatal).
...
- **Reconciliation:** replace §8's disposition list with the **proven 5 combinable labels** (more YAGNI-true, grounded in real precedent). Keep **Rule 0** (injection guard). Add the **INDEX-as-close-record** convention, and a lightweight **"processed, retained"** marker for owner-retained local-only items — which kills the stale-rediscovery risk for e.g. `history-data-analysis-2026-07-09.md`, still sitting in `_inbox` today.