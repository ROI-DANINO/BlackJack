...
| **1.4** `:83` | **Simulation alone does not produce understanding.** Three-cycle classroom programme: simulation software produced *disappointing* conceptual gains; gains appeared only when the activity forced students to **confront their own prediction against the simulated result**. ~~`[DEFECTIVE-SOURCE — see §0]`~~ **(tag lifted to `[VERIFIED]` 2026-08-17 at `LDB-10`, Ruling 1 — the tag was stale the day the bridge was written: its defect `C-C7-007` is present in the dossier at the original bank `55f24aa`.)** | **`[VERIFIED]`** | F11 (delMas) | "**a 'watch 10,000 hands play out' visualisation will not teach variance.** A predict-then-reveal-then-reconcile loop might. Predict first, then show." |
...
| **1.6** `:101` | **Frequency formats help, with a firm ceiling.** Natural frequencies beat probabilities — but roughly **three quarters still fail**, and ~half quietly translate frequencies back into probabilities. "In a sample *more* schooled than your users." ~~`[DEFECTIVE-SOURCE — see §0]`~~ **(tag lifted to `[VERIFIED]` 2026-08-17 at `LDB-10`, Ruling 1 — repaired by the ten `C-C7T` corrections, landed `6da7e9f` 2026-07-26; Weber reopened in full per `V-C7-topup.md:50`.)** | **`[VERIFIED]`** | F17 (Weber) | "**Use frequency framing; do not expect it to carry the concept on its own.**" |
...
> **Tag-integrity flags for the owner — BOTH RESOLVED 2026-08-17 at `LDB-10`.** Kept as a record of
> what was flagged and how it closed, rewritten from transient to terminal so no later reader
> re-opens a settled question. Each resolution below was checked in the target file, not in a
> reviewer record.
> 1. ~~**§1.4's `[DEFECTIVE-SOURCE]` tag looks stale.**~~ **CONFIRMED STALE, AND LIFTED.** The flag was
>    right. §1.4's source is F11 (delMas), whose defect was **C-C7-007**, and that correction is
>    present in the dossier at the original bank `55f24aa` — the same commit that authored the
>    bridge, so the tag was stale the day it was written. Ruling 1 at the `LDB-10` gate lifts §1.4
>    and §1.6 to **`[VERIFIED]`**, and `[DEFECTIVE-SOURCE]` is now unused in the bridge. The flag's
>    second horn — that the tag pointed at the wrong finding — is answered: **F19/C-C7T-008 is also
>    repaired**, landed `6da7e9f` 2026-07-26 with the other nine `C-C7T` corrections, so neither
>    horn leaves anything unrepaired behind it.
> 2. ~~**§2.3 has silently absorbed C-C7T-002 while the dossier has not.**~~ **NO LONGER TRUE — the
>    dossier caught up, and had already done so when this flag was written.** Both corrections
>    landed `6da7e9f` 2026-07-26: **C-C7T-002** at `C7-probability-ev-variance.md:1258`, and the
>    word "graded" is struck in the F16 headline at `:1257`; **C-C7T-004**'s independence point at
>    `:1358`. The bridge is no longer *ahead of* the dossier — they agree, so there is nothing left
>    to reconcile. *(This item was stale for the same reason §1.4's tag was, and it survived the
>    2026-08-17 pass that corrected the rest of this page — the eighth drift check in
>    `scripts/check-doc-drift.sh` reads the tag columns above but cannot read prose like this, a
>    limit it prints in its own output.)*
