...
>
> **CORRECTIONS-LANDING PASS (C1-EC2, 2026-07-26).** Five Phase-1 verifier corrections to F1–F21 —
> raised by V1 and V1c on 2026-07-19 and never written into this dossier — are now landed in place:
> the F9/F12 Knewton vendor non-disclosure (V1-C5, V1c-E), Pelánek §3.1's "10 answers per student"
> datum missing from F8 (V1-C6), F16's headline overstatement (V1c-B), F21a's tier Q2→Q4 (V1c-C), and
> F21b's edition misattribution (V1c-D). Each correction is labelled at its site with this pass's tag.
> No source was added or removed; no finding ID changed.
...
- Caveats: This paper is the strongest single source for sub-question 4, but its "opponent" framing for education-Elo is student-vs-**item**, not student-vs-peer — it does not describe Elo as literally requiring a pool of other learners to play against, contrary to how the brief frames it. It does, however, still require a population of many students' responses to fit item difficulty, so the "no opponent pool" framing in the brief needs this nuance. **[C1-EC2, 2026-07-26 — material passage the dossier missed, cutting *toward* feasibility; per V1's Correction C6.]** Pelánek **§3.1** states: "Once the Elo rating system has good estimates of item difficulties, it can estimate the skill of new students rather quickly — under the simulation settings just 10 answers per student are sufficient to produce reasonable skill estimates (correlation 0.8 with the ground truth)." This is directly on the load-bearing Q4 question. It establishes that the ~100-student requirement attaches to **calibrating item difficulty**, *not* to estimating an individual learner's skill once difficulties are fixed — precisely the product path this dossier's own sufficiency statement proposes ("using literature-typical or expert-set default parameters instead of population-fit ones"). Tier **Q3** (result of the paper's own simulation study, §3.1, an experimental result, unlike the 100-student figure).
...
- Caveats: This is a two-sentence framing statement in a paper's introduction, presented as settled background rather than a result the paper itself tests — treat as an authoritative summary claim, not as independently re-derived evidence. **[C1-EC2, 2026-07-26 — undisclosed vendor affiliation, per V1's Correction C5.]** The dossier does not record that both authors are Knewton, Inc. employees (a commercial adaptive-learning vendor). Low risk for a neutral structural claim about IRT vs BKT, but it belongs in the record — and the same vendor also supplies F12 (see V1c-E below); do not count F9 and F12 as two independent sources.
...
- Caveats: Two of this paper's four authors (Karklin, Ekanadham) also authored the T-SKIRT paper the dossier already cites as F9 — this finding reaches their empirical result, not only their background framing (the specific coverage failure the sufficiency review's M1 named). "Tractable" is load-bearing: IRT's advantage held only up to the granularity IRT could feasibly be trained on; the paper itself flags scaling difficulty for richer IRT variants beyond that point. **[C1-EC2, 2026-07-26 — Knewton vendor non-disclosure, per V1c-E.]** Three of four authors are **Knewton, Inc.** employees, and one of the three datasets is Knewton's own proprietary data. V1's Correction C5 already flagged the identical undisclosed Knewton affiliation on F9 (Ekanadham & Karklin). The same vendor now supplies **two** of this dossier's findings, one of which is a competitive benchmark asserting that the vendor's preferred model family beats the rival approach. The result is peer-reviewed at EDM 2016 and independently echoed by F11 and F13, so it does not fall — but a synthesis that stacks F9 and F12 as two independent sources would be double-counting one vendor.
...
**[C1-EC2, 2026-07-26 — SUPERSEDED HEADLINE. Narrowed per V1c-B.]** An "explanatory IRT" method that estimates a new learner's starting ability from background information and past trajectories of learning rather than that learner's own accumulated response history substantially reduced cold-start ability-estimation error **in a simulation study**; the real-data component is an unquantified illustration, and the covariate set (age, gender, learning disability, past trajectory) is not characterized in the accessible text.
- Provenance: PUBLISHED
...
- Proposed status bucket: Evidence-backed
- Proposed evidence-quality tier: ~~Q2 for Schroeders & Gnambs (peer-reviewed methodological tutorial synthesizing multiple simulation studies with quantitative thresholds by model type, in Advances in Methods and Practices in Psychological Science)~~ **[C1-EC2, 2026-07-26 — TIER DOWNGRADE, per V1c-C.]** **Q4** for Schroeders & Gnambs — a peer-reviewed tutorial that narratively summarizes other authors' simulation studies (König et al. 2020, Sheng 2013, Cuhadar 2022) is a narrative synthesis, not a primary controlled experiment; every quantitative figure in the cited passage is attributed to a third party. The claim itself is unaffected. Q3-and-informal for Linacre ~~(a practitioner guideline note in a technical newsletter, not a peer-reviewed article)~~ **[C1-EC2, 2026-07-26 — TIER DOWNGRADE, per V1c-D.]** **Q4**, not Q3 — this column reports no study of its own; it derives its numbers analytically from standard-error algebra and states them as guidance — recorded as mixed, matching how the dossier already handles mixed-tier sources (see F8)
- Source(s): (1) Schroeders, U. & Gnambs, T. (2025). "Sample-Size Planning in Item-Response Theory: A Tutorial." Advances in Methods and Practices in Psychological Science, 8(1), 1–13. DOI: 10.1177/25152459251314798 — https://timo.gnambs.at/sites/default/files/gnambstimo/publications/schroeders2025.pdf (2) ~~Linacre, J.M. (1994). "Sample Size and Item Calibration Stability." Rasch Measurement Transactions, 7(4), 328~~ **[C1-EC2, 2026-07-26 — EDITION CORRECTED, per V1c-D: must not be cited as a 1994 source.]** Linacre, J.M. "Sample Size and Item Calibration Stability." Rasch Measurement Transactions, 7(4), 328 (1994) — cite as rasch.org, current revision, descended from Linacre 1994, RMT 7:4 p.328. The live page cites Kruyen (2012) and is "Confirmed by Azizan et al. (2020)," so it has been materially revised at least twice since 1994; the quoted sentences are on the *current* page, and that they appeared in the 1994 print column is **not established**. — https://www.rasch.org/rmt/rmt74m.htm
